...
export function extractPrincipiosValores(text: string): PrincipioValor[] {
  ...
  const inicioMatch = normalized.match(/los\s+principios\s+y\s+valores\s+que\s+se\s+acogen\s+para\s+el\s+programa[^:]*:\s*/i);
  if (!inicioMatch) {
    return principios;
  }
  ...
  const finMatch = normalized.substring(startPos).match(/\n\d+\.\s+[A-ZÁÉÍÓÚÑ]/);
  ...
  const principioPattern = /([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+(?:y\s+)?[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)*)[:\.]\s+([^\n]+(?:\n(?![A-ZÁÉÍÓÚÑ][a-záéíóúñ]+[:\.]).*?)*?)(?=\n[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+[:.]|\n\d+\.|\n\n\d+|$)/g;
  
  let match;
  const maxPrincipios = 15;
  let count = 0;

  while ((match = principioPattern.exec(seccionPrincipios)) !== null && count < maxPrincipios) {
    ...
    const noEsFalsoPositivo = !['Laboratorio', 'Facultad', 'Departamento', 'Coordinador'].includes(nombre);
